...
function OrdersPage({ lang, t, conversations, setConversations }: { lang: Lang; t: (kr: string, en: string) => string; conversations: Conversation[]; setConversations: React.Dispatch<React.SetStateAction<Conversation[]>> }) {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [tab, search, needsResponseSellerIds, sortKey, sortDir, conversations]);

  const tabCounts = useMemo(() => {
    const c: Record<string, number> = {};
    TABS.forEach((tb) => {
      if (tb.id === "needs_response") {
        c[tb.id] = ALL_ORDERS.filter((o) => needsResponseSellerIds.has(o.seller.id)).length;
      } else {
        const map = TAB_MAP[tb.id as TabId];
        c[tb.id] = !map ? ALL_ORDERS.length : ALL_ORDERS.filter((o) => map.includes(o.status)).length;
      }
    });
    return c;
  }, [needsResponseSellerIds]);
